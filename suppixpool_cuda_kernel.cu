    ...
    const int K)
{
    ...
    */
    ...
    // 先创建一个和输入一样大小的 index 矩阵，得到index矩阵后再填充
...
template <typename scalar_t>
__global__
void spx_ave_pooling_forward_kernel(
    const scalar_t* __restrict__ image,
    const int* __restrict__ labels,
    int* pool_size,
    const int imWidth,
...
// aveNum: denotes spix size, saved for backward
    ...
    const int K)
{
    ...
    */
    ...
    AT_DISPATCH_FLOATING_TYPES(img.type(), "spx_ave_pooling_forward_cuda", ([&] {
    spx_ave_pooling_forward_kernel<scalar_t><<<blocks, threads>>>(
        img.data<scalar_t>(),
        spx_labels.data<int>(),
        pool_size.data<int>(),
        imW,
